...
class TGCN(nn.Module):
    def __init__(self,
        ...
        self.embed_layer = RelGraphFeat(pub_node_feats=pub_node_feats,
    ...
    def forward(self, input_nids,blocks,feat):
        input_nids={k:input_nids[k].to(self.device) for k in input_nids.keys()}
        feat={k:feat[k].to(self.device) for k in feat.keys()}
        blocks=[blk.to(self.device) for blk in blocks]
        h=self.embed_layer(input_nids,feat)
    ...
    def inference(self,test_loader,feat,labels,pub_node_ids,downstream,device):
        total_acc=0.0
        count=0
        ...
        for input_nodes, output_nodes, blocks in tqdm.tqdm(test_loader):
            ...
            h=self.embed_layer(input_nodes,h)
            for layer, block in zip(self.layers, blocks):
    ...
    def inference_on_glb(self,demo_graph:DGLHeteroGraph,test_loader,labels,downstream,device):
        ...
        for input_nodes, output_nodes, blocks in tqdm.tqdm(test_loader):
            ...
            input_nodes={k:input_nodes[k].to(device) for k in input_nodes.keys()}
            h=self.embed_layer.inference(input_nodes,h)
