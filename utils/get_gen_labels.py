...
from train.mb_eval_rgcn import data_prep,set_loaders_light,map_nid_to_id
from utils import dblp_config
import numpy as np

def get_prime_graph(hidden_portion:float,graph:DGLHeteroGraph,star_ntype='paper'):
    graph=graph.to('cpu')
    if graph.device!='cpu':
    ...
    # print("possible_nodes = "+str(possible_nodes))
...
def get_tngen_labels(whole_g:DGLHeteroGraph,cur_subg:DGLHeteroGraph,downstream:str):
    ...
    etypes=list(dblp_config.pred_in_n_rev_etypes.keys())
            ...
            if e_i in cur_subg.canonical_etypes:
                if e_i[-1]==downstream:
                    ...
                    if miss>0:
                        ...
                        for d_i in diff[:min(len(diff),dblp_config.num_pred)]:
                            if whole_g.nodes[e_i[0]].data['feat'][d_i] is None or len(
                                    whole_g.nodes[e_i[0]].data['feat'][d_i]) != dblp_config.feat_len:
                                feats.append(torch.zeros(dblp_config.feat_len))
                            else:
                ...
                if e_i[-1]==downstream:
                    ...
                    if miss>0:
                        ...
                        feats = []
                        for n_i in miss_gids[:min(len(miss_gids),dblp_config.num_pred)]:
                            if whole_g.nodes[e_i[0]].data['feat'][n_i] is None or len(
                                    whole_g.nodes[e_i[0]].data['feat'][n_i]) != dblp_config.feat_len:
                                feats.append(torch.zeros(dblp_config.feat_len))
                            else:
...
def rdm_get_prime_graph(hidden_portion:float,graph:DGLHeteroGraph,node_type,device='cpu'):
    ...
    for e_i in list(dblp_config.pred_in_n_rev_etypes.keys()):
